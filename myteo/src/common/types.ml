...
let get_all_tvars ty =
  let tvars_with_duplicates = get_all_tvars_with_duplicates ty in
...
let rec pp_with_names ~tvar_to_name ty =
  ...
  | String -> "string"
  | Function { params; return } ->
    List.map (pp_with_names ~tvar_to_name) params
    |> String.concat " -> "
    |> ( ^ ) " -> "
...
let pp ?(tvar_to_name = IMap.empty) ?(use_tvar_ids = false) ty =
  let all_tvars = get_all_tvars ty in
  let used_names = IMap.fold (fun _ name acc -> SSet.add name acc) tvar_to_name SSet.empty in
  let rec build_tvar_to_name (tvar_to_name_acc, names_acc) next_name_id tvar_ids =
    match tvar_ids with
    | [] -> (tvar_to_name_acc, names_acc)
    | hd :: tl ->
      if IMap.mem hd tvar_to_name_acc then
        build_tvar_to_name (tvar_to_name_acc, names_acc) next_name_id tl
      else
        ...
        build_tvar_to_name
          ...
          tl
  in
  let (tvar_to_name, _) = build_tvar_to_name (tvar_to_name, used_names) 0 all_tvars in
  pp_with_names ~tvar_to_name ty
