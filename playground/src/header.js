...
function SplitDropdownButton(props) {
  ...
  return (
    <div className="headerSplitDropdownButton">
      <button className="headerSplitLeftButton button" onClick={props.onClick}>
        {props.value.label}
        {PLAY_ICON_SVG}
      </button>
      <div className={`dropdown ${isDropdownActive ? "is-active" : ""}`}>
        <div className="dropdown-trigger">
          <button
            className="headerSplitRightButton button headerEllipsisButtonElement"
            aria-haspopup="true"
        ...
        <div className="dropdown-menu" id={props.id} role="menu">
          <div className="dropdown-content">
            {props.values.map((command) => (
              <a
                key={command.id}
                className="dropdown-item"
                onClick={() => props.onChange(command)}
              >
                {command.label}
              </a>
            ))}
  ...
  return (
    <div className="header">
      <a className="headerBrand" href="/">
        <img className="headerLogo" src="myte-logo.svg" />
      ...
      <SplitDropdownButton
        ...
        value={props.command}
        values={COMMANDS}
        onChange={props.onChangeCommand}
        onClick={props.onSubmit}
      />
      <button className="headerButton button">Settings {GEAR_ICON_SVG}</button>
    </div>
  );
}
